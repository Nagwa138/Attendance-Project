...
class EmployeeController extends Controller
{
            ...
            if ($active->count() > 0) {
                foreach ($active as $a) {
                    ...
                    if ($a->active == 1) {
                        ...
                        $users = User::all();
                        return view('dashboard.employee.guest' , compact('users'));
                    }
                }
            }
        } else {
    ...
    public function addMember(Request $request){
        ...
        if ($request->pic)
        {
            ...
            \Intervention\Image\Facades\Image::make($request->pic)->resize(300, null, function ($constraint) {
                $constraint->aspectRatio();
            })->save(public_path('uploads/users_images/'. $request->user_id .'/'. $request->pic->hashName()));

            $request_data['pic'] = $request->pic->hashName();
